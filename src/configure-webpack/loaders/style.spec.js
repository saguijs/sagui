...
describe('style', function () {
  ...
  it('should have css modules enabled by default', () => {
    ...
    expect(config.module.loaders[0].loader.includes('css-loader?modules')).to.eql(true)
    expect(config.module.loaders[1].loader.includes('css-loader?modules')).to.eql(true)
  })

  it('should be possible to disable css modules', () => {
    const config = loader.configure({ projectPath, style: { cssModules: false } })

    expect(config.module.loaders[0].loader.includes('css-loader?modules')).to.eql(false)
    expect(config.module.loaders[1].loader.includes('css-loader?modules')).to.eql(false)
  })
