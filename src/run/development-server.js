    ...
    try {
      ...
      server.listeningApp.on('error', function(e) {
        if (e.code === 'EADDRINUSE') {
          logError(
            `Server failed to started at http://localhost:${saguiConfig.port}. Port already used.`
          )
...
function concatHMRBundle(entry) {
  const devClient = [require.resolve('webpack-dev-server/client/') + '?/', 'webpack/hot/dev-server']
