...
const saguiScripts = {
  format: ['sagui format'],
  build: ['sagui build'],
  develop: ['sagui develop --port 3000', undefined],
  dist: [
  ...
  start: ['npm run develop', 'sagui develop --port 3000'],
  test: [
  ...
  'test:coverage': ['npm run test:unit -- --coverage', undefined],
  'test:lint': ['sagui lint', 'sagui test:lint', undefined, 'sagui test:lint'],
  'test:typecheck': ['sagui typecheck', 'sagui test:typecheck'],
  'test:unit': [
...
export default function(userScripts = {}) {
  const combinedScripts = uniq([...Object.keys(saguiScripts), ...Object.keys(userScripts)]).sort()
  ...
  return combinedScripts.reduce((scripts, key) => {
    ...
    if (
      (hasAnOutdatedScript && latestScriptIsDefined) ||
      (isMissingScript && latestScriptIsDefined)
    ) {
